...
from doclens.answer import RETRIEVAL_MODES, answer_question
...
def run_eval(
    ...
    *,
    ...
    """Run eval over gold cases x modes, resume by (model, mode, case_id), atomic writes.

    Args:
        models: List of chat model names.
        gold: List of gold cases from load_gold.
        out_path: Path to results.json (create if missing).
        modes: Retrieval modes to evaluate side by side, e.g. `RETRIEVAL_MODES`
            (`"dense"`, `"hybrid"`, `"hybrid_rerank"`). The corpus (and its
            HybridIndex) is built once per doc and reused across every mode.
    ...
    # Build resume set: {(model, mode, case_id) for already-run}. `.get("mode")`
    # tolerates pre-mode-axis records (they'll simply never match a real mode
    # and so get re-run rather than silently skipped).
    resume_set = {(r["model"], r.get("mode"), r["case_id"]) for r in results["records"]}

    # Compute pending (model, mode, case_id) triples
    pending_triples = set()
    for model in models:
        for mode in modes:
            for case in gold:
                if (model, mode, case["id"]) not in resume_set:
                    pending_triples.add((model, mode, case["id"]))
    ...
    # Only for docs referenced by pending cases; built once and reused across
    # every mode (embedding cost does not scale with len(modes)).
    pending_case_ids = {triple[2] for triple in pending_triples}
    ...
    # Run evals per (model, mode, case)
    for model in models:
        chat, chat_model = chat_factory(model)
        for mode in modes:
            for case in gold:
                if (model, mode, case["id"]) in resume_set:
                    continue
                ...
                try:
                    res = answer_question(chat, chat_model, embedder, embed_model,
                                        index, question, k=5, retrieval_mode=mode)
...
def main():
    """CLI: python -m evals.run --models a,b --out results.json [--gold path]
    [--modes dense,hybrid,hybrid_rerank] [--sleep 2]
    """
    ...
    parser.add_argument("--modes", type=str, default=",".join(RETRIEVAL_MODES),
                        help="Comma-separated retrieval modes "
                             f"(default: {','.join(RETRIEVAL_MODES)}).")
    ...
    results = run_eval(models, gold, args.out, modes=modes, sleep_s=args.sleep)
